...
export default function PublicServicePreviewPage({ uuid }: { uuid: string }) {
  const { data: service, isLoading: isServiceLoading } =
    useGetSinglePublicService(uuid);

  const isAuthenticated = useAppSelector((state) => state.auth.isAuthenticated);

  return isServiceLoading ? (
    <ServicePreviewPageSkeleton />
  ) : service ? (
    <ServicePreviewMainView
      service={service}
...
function ServicePreviewMainView({
  service,
  isAuthenticated,
}: {
  ...
  const handleBookNow = () => {
    ...
    router.push(`/login?next=/services/${service.id}/preview`);
  };
      ...
      <Stack spacing={3}>
          ...
          >
            ...
            >
              ...
              <Button
                variant="contained"
                startIcon={
                  isAuthenticated ? <CalendarMonthRounded /> : <LoginRounded />
                }
        ...
        >
          ...
          <CardContent sx={{ p: { xs: 2.5, sm: 3 } }}>
            {activeTab === 0 && <ServiceOverviewTab service={service} />}
            {activeTab === 1 && <BookingInfoTab service={service} />}
          </CardContent>
        </Card>
      </Stack>
    </Box>
...
function ServiceOverviewTab({ service }: { service: CompanyServices }) {
  return (
    <Stack spacing={3}>
      <Box>
        <Typography
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 4 }}>
          <DetailInfoCard
            icon={<Category />}
            label="Category"
            value={service.category_name || service.category}
          />
        </Grid>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 4 }}>
          <DetailInfoCard
            icon={<Business />}
            label="Provider"
            value={service.company_name || service.company}
          />
        </Grid>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 4 }}>
          <DetailInfoCard
            icon={<EventAvailable />}
            label="Availability"
            value="Check available times when booking"
          />
        </Grid>
      </Grid>
    </Stack>
...
function BookingInfoTab({ service }: { service: CompanyServices }) {
  return (
    <Stack spacing={3}>
      <Box
        sx={{
      ...
      >
        ...
        >
          ...
          <strong>{formatDuration(service.duration_minutes)}</strong>. You can
          choose an available time slot during the booking process.
        </Typography>
      </Box>
...
function ServicePreviewPageSkeleton() {
  return (
    <Box
      sx={{
        width: "100%",
        ...
        <CardContent sx={{ p: { xs: 3, sm: 5 } }}>
          <Stack spacing={2.5} alignItems="center">
            ...
            <Box>
              ...
              >
                This service may no longer be available, or it may be
